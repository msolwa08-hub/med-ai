...
- THE QUESTION TYPE MUST MATCH THE ANSWER TYPE — this is a hard rule, get it wrong and the instrument is unusable at the bedside:
  ...
  · A free-text answer in the patient's own words (an indication for a prior procedure, a medication name, "what type of surgery", "what did the ultrasound say") is NEVER a yes/no tick and never num. Use kind:"text" — no eff needed. Example: {"id":"gynsurgwhy","lbl":"Indication for surgery","stream":"hx","kind":"text"}.
...
- DEMOGRAPHIC-SPECIFIC HISTORY: for obstetric or neonatal presentations, put the antenatal history in PT.background (gravidity/parity, gestation, antenatal complications, booking bloods / serology, relevant scans) and include antenatal-relevant findings. Reflect age, sex and pregnancy in PT.line so vital-sign ranges are judged correctly (e.g. "8 mo · ♀ · fever", "28 · ♀ · 32/40 · ...", "72 · ♂ · ...").
...
- FOLLOW-UP CASCADE: when a symptom has clinically-important qualifiers, give it a "follow" array of child findings that are only asked once the parent is present — e.g. cough → {productive → {colour change, blood}}, or chest pain → {radiates to arm, worse on exertion}. Each child is a normal finding {id, lbl, stream} and, if it discriminates a differential, eff (maps dx id to LR+/LR-) — omit eff for a child that is purely informational (free text or an unthresholded number). A child MAY itself have "follow" for deeper nesting, and MAY be kind:"text" or num per the question-type rule above. Use this to characterise a positive rather than listing every qualifier as a separate top-level finding.
