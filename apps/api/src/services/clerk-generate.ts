...
- QUANTIFY where a number matters: if a finding is defined by a threshold (menstrual loss, cycle length, bleeding duration, weight loss, fever, urine output, GCS, etc.), add a "criterion" (the clinical cutoff, e.g. "≥ 80 mL per cycle", "> 8 days", "cycle < 24 or > 38 days") AND an "ask" (a plain-language, patient-facing translation of that cutoff, e.g. "soaking a pad every ≤ 2 h, flooding, or clots > 2.5 cm") so the intern knows what counts as positive and how to ask the patient in ordinary words. When the value is a discrete number the clinician should type (cycle length, bleeding days, GCS), also add num:{unit, lo?, hi?} — abnormal when the value is below lo or above hi.
...
- MX: terse management. investigate[] vs treatment is a HARD split: diagnostic tests (CT, ECG, bloods, βhCG, diagnostic LP, X-ray, ultrasound) are NOT treatments — never put them in immediate[]/definitive[] as rx cards. Put the time-critical ones in investigate[] as {lbl, when, ix:'<matching IX id>'} (e.g. {"lbl":"Non-contrast CT head","when":"within 1 h","ix":"ct"}); ix must reference an id that exists in IX. immediate[] and definitive[] are for TREATMENTS/interventions only — drugs, fluids, and procedures that TREAT (a therapeutic endoscopy that bands/clips, surgery, delivery stay here even though they also diagnose). immediate[] and definitive[] items are {rx, for, dose, sign?:true, active?:'why now', ind?:'why indicated'}. CRITICAL: definitive[] MUST LEAD with the mainstay / definitive treatment of the SINGLE most likely diagnosis (the first DX) — the actual drug or procedure that TREATS it (e.g. for migraine: triptan + NSAID + antiemetic, and prevention if frequent) — not only steps to rule out or empirically cover the must-not-miss rivals. Put the leading diagnosis's own treatment first, then the rival-directed ("only if X confirmed") steps. The plan must never treat the killers and forget the actual diagnosis. monitor[] is strings. levers[] are treat-and-see items {give, resp, means}. holistic[] is the NON-DRUG layer for the leading diagnosis — rows of {label, chips:[short items]} (or {label, note}) covering: trigger avoidance (name the concrete triggers, e.g. chocolate, aged cheese, red wine, caffeine, skipped meals, poor sleep, stress), lifestyle change, prevention, self-management, and patient education / safety-netting. Include holistic whenever the leading diagnosis has meaningful non-drug management (most chronic or recurrent conditions do); [] only for purely acute one-off surgical problems. DOSING — BE SPECIFIC: for well-established first-line drugs, give the standard adult dose, route and frequency exactly as an intern would find it in the STG / BNF (e.g. "ceftriaxone 2 g IV 12-hly", "sumatriptan 50–100 mg PO", "aspirin 300 mg PO stat", "enoxaparin 1 mg/kg SC 12-hly"). Use weight-based mg/kg for paediatric or weight-dependent drugs (e.g. "paracetamol 15 mg/kg PO 6-hly"). Only write "per protocol"/"titrate" when the dose genuinely depends on a local titration protocol. ALWAYS set sign:true on every drug so a clinician signs off, and never state a dose you are not confident is the accepted standard — omit the drug rather than guess. Cite nothing you are unsure of.
