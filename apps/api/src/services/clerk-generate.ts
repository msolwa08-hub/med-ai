...
- FOLLOW-UP CASCADE: when a symptom has clinically-important qualifiers, give it a "follow" array of child findings that are only asked once the parent is present — e.g. cough → {productive → {colour change, blood}}, or chest pain → {radiates to arm, worse on exertion}. Each child is a normal finding {id, lbl, stream, eff} (eff maps to dx ids, LR+/LR-) and MAY itself have "follow" for deeper nesting. Use this to characterise a positive rather than listing every qualifier as a separate top-level finding.
