...
const DEPT_CLINICAL_GUIDANCE: Record<string, string> = {
  ...
  family: 'Family Medicine / Primary Health Care discipline: reason from the WHOLE PATIENT, not the complaint in isolation — the presenting symptom lives inside a chronic-disease landscape (HPT, DM, HIV, TB, epilepsy, asthma/COPD, mental health) and the consultation must address BOTH. Chronic disease management is the spine: at every visit confirm the register (which chronic conditions, on what treatment, last review dates), assess CONTROL (BP target per SA STG — <140/90 general, <130/80 if DM/CKD; HbA1c <7%; VL suppressed), screen for complications (diabetic foot, proteinuria, retinopathy referral), and interrogate ADHERENCE with curiosity not judgement (pill counts, pharmacy refill dates, barriers — transport, stigma, side-effects, food insecurity). The PHC screening reflex runs in parallel: HIV test if unknown or >12 months, TB symptom screen (cough/fever/night sweats/weight loss — any one → sputum GeneXpert), cervical cancer screening (Pap per policy), mental health (PHQ-2 → PHQ-9 if positive), BMI + lifestyle. SA STG is the formulary — prescribe from the PHC Essential Medicines List (amlodipine/HCTZ not branded ARBs, metformin first-line not SGLT2 at PHC level); know the step-up thresholds and the referral triggers (e.g. BP uncontrolled on 3 agents → refer, HbA1c >10% on max oral → insulin initiation or refer). STI syndromic management per national guidelines (vaginal discharge, urethral discharge, genital ulcer — treat the SYNDROME, partner notification). Contraception counselling at every reproductive-age visit. Social determinants are clinical data: employment, grants (CSG/DG/OAG), food security, housing, substance use, GBV screening — these predict outcomes more than the prescription. The REFERRAL DECISION is the key output: what you manage at PHC level vs what needs district/regional hospital input, and what the patient needs to bring (referral letter, current meds, recent results). Continuity of care: document for the NEXT visit — what was changed, what to check, when to return.',
