...
export function assessmentAssistFields(d: AssessmentData, dept: DeptId, subDept?: string): AssistField[] {
  const base: AssistField[] = [
    ...
    { key: 'examination', label: 'Focused Examination', value: d.examination, kind: 'textarea', hint: 'driven by the history — examine the systems the presenting complaint implicates (CVS, Resp, Abdo, Neuro), and record the RELEVANT NEGATIVES, not just the positives', placeholder: 'Systems relevant to the complaint — with the pertinent negatives' },
