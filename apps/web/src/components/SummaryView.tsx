...
export function SummaryView({ session, onBack, onBackToList, onApprove, onSaveNotes, onNewPatient }: Props) {
  ...
  const patientMessages = session.messages.filter((m) => m.role === 'patient');
  const label = patientMessages.length > 0
    ? patientMessages[0].content.split(' ').slice(0, 5).join(' ') + '…'
    : session.label;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10 print:hidden">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-gray-400">Clinical Summary</div>
          </div>
          <div className="flex items-center gap-1.5 flex-shrink-0">
            {summary && (
              <>
                <button
                  onClick={handleCopy}
                  className="text-xs text-gray-500 hover:text-gray-700 flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 transition"
                >
                  {copied ? (
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        {/* Approval banner */}
        {!session.approved && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 mb-5 flex items-start gap-3 print:hidden">
            ...
            <button
              onClick={onApprove}
              className="flex-shrink-0 px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold rounded-lg transition"
            >
              Approve
            </button>
          </div>
        ...
        {session.approved && (
          <div className="bg-green-50 border border-green-200 rounded-xl px-4 py-3 mb-5 flex items-center gap-3 print:hidden">
            <svg className="w-5 h-5 text-green-600 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <p className="text-sm text-green-700 font-medium">Approved by Dr. Patel</p>
          </div>
        )}
        ...
        {tab === 'summary' && (
          <div className="print-area">
            ...
            <div className="hidden print:block mb-6">
              ...
              <p className="text-sm text-gray-600">Sandton Family Practice · {sessionDate}</p>
              <p className="text-sm text-gray-600">Patient: {session.label}</p>
              <hr className="mt-3" />
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="bg-blue-600 px-6 py-4 print:hidden">
                ...
                <p className="text-blue-200 text-sm mt-0.5">{sessionDate}</p>
              </div>
            ...
            <div className="mt-5 print:mt-6">
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                <label className="block text-sm font-semibold text-gray-800 mb-2">Clinician notes</label>
                <textarea
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="Add your clinical notes here…"
                  rows={4}
                  className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none print:hidden"
                />
                {notes && (
                  <div className="hidden print:block text-sm text-gray-800 whitespace-pre-wrap">{notes}</div>
                )}
            ...
            <div className="mt-5 flex flex-col sm:flex-row gap-3 print:hidden">
              <button
                ...
                className="flex-1 py-3 px-4 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-medium rounded-xl transition"
              >
                Back to sessions
              </button>
              <button
        ...
        {tab === 'transcript' && (
          <div>
              ...
              {session.messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'patient' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <div className="flex flex-col gap-1 max-w-[80%]">
                    ...
                    <span className={`text-xs text-gray-400 ${msg.role === 'patient' ? 'text-right' : 'text-left'}`}>
            ...
            <div className="mt-5 print:hidden">
              <button
                onClick={onBackToList}
                className="w-full py-3 px-4 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-medium rounded-xl transition"
              >
                Back to sessions
              </button>
            </div>
