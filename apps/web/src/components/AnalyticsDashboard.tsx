    ...
    <div className="space-y-6">
      ...
      {Object.keys(analytics.byDepartment).length > 0 && (
        <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
          <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">By Department</h3>
          <div className="space-y-3">
            {Object.entries(analytics.byDepartment)
              .sort(([, a], [, b]) => b - a)
              .map(([dept, count]) => (
                <div key={dept} className="flex items-center gap-3">
                  <span className="text-slate-300 text-sm w-32 shrink-0">{DEPT_LABELS[dept] ?? dept}</span>
