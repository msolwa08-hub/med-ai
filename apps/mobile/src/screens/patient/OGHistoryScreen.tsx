...
type OGMode = 'OBSTETRIC' | 'GYNAECOLOGICAL' | 'UNKNOWN';
...
const OG_MODE_LABELS: Record<OGMode, { label: string; icon: string; color: string }> = {
  OBSTETRIC:    { label: 'Obstetric History', icon: 'heart-outline',    color: COLORS.systemPurple },
  GYNAECOLOGICAL: { label: 'Gynaecological History', icon: 'medical-outline', color: COLORS.secondary },
  UNKNOWN:      { label: 'O&G History',        icon: 'woman-outline',   color: COLORS.primary },
};

const STAGE_LABELS_OBS = [
  'Presenting', 'Current Pregnancy', 'ANC', 'Blood Results',
  'Obstetric History', 'Medical Hx', 'Social Hx', 'Summary',
];
const STAGE_LABELS_GYN = [
  'Complaint', 'Menstrual Hx', 'Contraception', 'Sexual Hx',
  'STI/Cervical', 'Pelvic Pain', 'Fertility', 'Summary',
];

function deriveStage(messages: Message[], mode: OGMode): number {
  const aiCount = messages.filter((m) => m.role === 'ai' && !m.isLoading).length;
  const labels = mode === 'OBSTETRIC' ? STAGE_LABELS_OBS : STAGE_LABELS_GYN;
  return Math.min(aiCount, labels.length - 1);
}
...
const StageIndicator: React.FC<{ stage: number; total: number; labels: string[] }> = ({
  stage,
  total,
  labels,
}) => (
    ...
    <View style={styles.stageDots}>
      {Array.from({ length: total }).map((_, i) => (
        <View
          ...
          style={[
            styles.stageDot,
            i <= stage ? styles.stageDotActive : styles.stageDotInactive,
          ]}
        />
      ))}
    </View>
...
export const OGHistoryScreen: React.FC = () => {
  const navigation = useNavigation<any>();
  const route = useRoute<RouteProp<RouteParams, 'OGHistory'>>();
  const {
    consultationId,
    language = 'en',
    isPregnant,
  ...
  const [ogMode, setOGMode] = useState<OGMode>('UNKNOWN');
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      const res = await apiClient.post('/og-history/start', {
        consultationId,
        language,
        isPregnant,
        chiefComplaint,
        gravida,
        para,
        mode: isPregnant ? 'OBSTETRIC' : 'UNKNOWN',
      });

      const data = (res.data as { data: { message: string; mode: OGMode; isComplete: boolean; redFlagDetected: boolean } }).data;
      setOGMode(data.mode);
      ...
      Alert.alert('Error', 'Failed to start O&G history session. Please try again.', [
        { text: 'OK', onPress: () => navigation.goBack() },
      ]);
    } finally {
      setIsLoading(false);
    }
  }, [consultationId, language, isPregnant, chiefComplaint, gravida, para, navigation]);
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await apiClient.post('/og-history/continue', {
        consultationId,
        patientMessage: text,
      });

      const data = (res.data as { data: { message: string; isComplete: boolean; redFlagDetected: boolean } }).data;
  ...
  const handleComplete = async () => {
    ...
    try {
      await apiClient.post('/og-history/complete', { consultationId });
      Alert.alert(
        'History Complete',
        'The O&G history has been compiled and sent to your doctor for review.',
  ...
  const modeConfig = OG_MODE_LABELS[ogMode];
  const stageLabels = ogMode === 'OBSTETRIC' ? STAGE_LABELS_OBS : STAGE_LABELS_GYN;
  const currentStage = deriveStage(messages, ogMode);
  ...
  const renderMessage = ({ item }: { item: Message }) => {
    ...
    return (
      <View style={[styles.messageRow, isAI ? styles.messageRowAI : styles.messageRowUser]}>
        {isAI && (
          <View style={[styles.avatarCircle, { backgroundColor: modeConfig.color }]}>
            <Ionicons name={modeConfig.icon as any} size={14} color={COLORS.white} />
          </View>
        )}
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerMid}>
          <View style={[styles.modeChip, { backgroundColor: modeConfig.color }]}>
            <Ionicons name={modeConfig.icon as any} size={12} color={COLORS.white} />
            <Text style={styles.modeChipText}>{modeConfig.label}</Text>
          </View>
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
        {!isStarted ? (
          <View style={styles.startingView}>
            <ActivityIndicator size="large" color={modeConfig.color} />
            <Text style={styles.startingText}>Starting O&G history session...</Text>
          </View>
        ) : (
          <FlatList
            ref={flatListRef}
        ...
        {!isComplete ? (
          <View style={styles.inputBar}>
            ...
            />
            <TouchableOpacity
              style={[styles.sendBtn, { backgroundColor: modeConfig.color }, (!inputText.trim() || isLoading) && styles.sendBtnDisabled]}
              onPress={sendMessage}
          ...
          <View style={styles.completeBanner}>
            {isExtracting ? (
              <>
                ...
                <Text style={styles.completeText}>Compiling your O&G history...</Text>
              </>
            ) : (
              <>
...
export default OGHistoryScreen;
